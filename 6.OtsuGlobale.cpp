...
void myOtsu(Mat& src, Mat& dest)
{
    ...
    */
    ...
    for (int i = 0; i < L; i++){
        ...
        globAVG += i * normHistogram[i]; //Calcola intensita globale media m_G 
    }
    /*
    2° Passo:
    ...
    Punto 6 - Calcolare la soglia k*, ovvero il valore k per cui 𝜎(2B)(k) è
    massimo
    */
    ...
    for (int i = 0; i < L; i++)
    {
        ...
        p2k = globAVG - p1k; //Differenza tra media ponderata su tutti i valori di intesita e media ponderata al passo i-esimo 
...
int main(int argc, char** argv)
{
    ...
    Mat original = imread("eye.jpeg", IMREAD_GRAYSCALE);
    if (original.empty())
    {
