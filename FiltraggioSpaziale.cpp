...
int main(int argc, char** argv) {
    ...
    Mat kernelLaplacian90 = Mat(3, 3, CV_32F, dataLaplacian90);
    ...
    float dataSobel_gx[] = { -1, -2, -1, 0, 0, 0, 1, 2, 1 };
    ...
    float dataSobel_gy[] = { -1, 0, 1, -2, 0, 2, -1, 0, 1 };
...
void checkMat(Mat my, Mat filter) {
    ...
    for (int i = 0; i < my.rows; i++) 
        for (int j = 0; j < my.cols; j++) 
            if ((int)my.at<float>(i, j) != (int)filter.at<float>(i, j))
                cont++;
